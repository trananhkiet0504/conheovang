      ...
      {isLoading && (
        <div className='fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-md transition-opacity duration-300 ease-in-out'>
          <div className='animate-fade-in flex flex-col items-center gap-4 rounded-2xl bg-white p-8 shadow-2xl dark:bg-gray-900'>
            {/* Spinner */}
            <div className='h-12 w-12 animate-spin rounded-full border-4 border-t-transparent border-r-indigo-500 border-b-transparent border-l-blue-500' />

            {/* Glowing text */}
            <p className='text-center text-sm font-medium text-gray-800 drop-shadow-sm dark:text-gray-200'>
              {message}
            </p>
          </div>
        </div>
